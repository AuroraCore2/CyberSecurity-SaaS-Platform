...
def run_detection(db: Session):
    ...
    2. Statistical Anomaly Detection (ML-lite)
    ...
    for log in logs:
        ...
        if log.source in ["web", "security", "application"] and log.severity in ["HIGH", "CRITICAL"]:
            ip_http_fails[ip] += 1
        
        # RULE 3: Port Scanning Detection
        if log.source == "firewall" and log.action and ("BLOCK" in log.action.upper() or "DENY" in log.action.upper()):
            if log.resource:
    ...
    # ========================================================================
    # PHASE 2: THRESHOLD ANALYSIS (Rule-Based Counting)
    # ========================================================================
    
    # RULE 1: SSH Brute Force Threshold
    ...
    # RULE 2: HTTP Brute Force Threshold
